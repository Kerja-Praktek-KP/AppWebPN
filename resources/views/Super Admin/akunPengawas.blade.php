...
<body class="bg-[#F2F3F9] overflow-hidden" x-data="{ sidebarOpen: true, isEditing: false, user: { nama: 'Arie', nip: '2104111010066', email: 'Arie@gmail.com', password: '******', jabatan: 'Pengawas', divisi: '-'}, editUser: {}, showPopup: false, showUserForm: true }">
    <!-- Header -->
    ...
    <div class="flex h-screen">
        ...
        <div :class="[sidebarOpen ? 'w-11/12' : 'w-full', 'bg-[#F2F3F9] ml-0 md:ml-28 lg:ml-16 xl:ml-60 flex-1 p-12 transition-all duration-300']">
            ...
            <form action="{{ route('users.editAkunProfil',  ['role' => 'Pengawas']) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
