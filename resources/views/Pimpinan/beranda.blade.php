                ...
                @foreach($targetUsers as $targetUser)
                    @if(Auth::user()->role === 'Pimpinan')
                        @if($targetUser->role == 'Koordinator Pengawas')
                            <a href="{{ route('penilaianDetailKoordinatorPengawasPimpinan') }}" class="bg-white p-2 rounded-lg hover:shadow-lg w-full transition ease-in-out delay-150 hover:-translate-y-0 hover:scale-105 hover:bg-white duration-300 flex flex-col justify-start items-center">
